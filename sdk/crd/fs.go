...
func CRDForObject(obj runtime.Object) (*apiextensionsv1.CustomResourceDefinition, error) {
	return CRDForGroupKind(obj.GetObjectKind().GroupVersionKind().GroupKind())
}

// CRDForGroupKind returns the CRD for the given group and kind or an error if there is no
// CRD available (i.e. any GK outside of *.k8c.io was provided).
func CRDForGroupKind(gk schema.GroupKind) (*apiextensionsv1.CustomResourceDefinition, error) {
	...
	kindPlural := strings.ToLower(flect.Pluralize(gk.Kind))
	filename := gk.Group + "_" + kindPlural + ".yaml"

	dir, err := getDirForGroup(gk.Group)
	if err != nil {
		return nil, fmt.Errorf("no CRD available \"%s_%s\": %w", gk.Group, kindPlural, err)
	}

	crd, err := loadCRD(dir, filename)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return nil, fmt.Errorf("no CRD available for \"%s_%s\"", gk.Group, kindPlural)
		}
